...
public class Experssion {
	...
	public static List<Word> wordAnalysis(String experssionStr){
		...
		for(int i=0;i<exChar.length;i++) {
			...
			if(exChar[i]=='+'||exChar[i]=='-'||exChar[i]=='*'||exChar[i]=='/'||exChar[i]=='('||exChar[i]==')') {
				...
				StringBuilder sb=new StringBuilder();
				while(('a'<=exChar[i]&&exChar[i]<='z')||('A'<=exChar[i]&&exChar[i]<='Z')||exChar[i]=='_') {
				...
				if(exChar[i]=='.') {
					++i;
					if(('a'<=exChar[i]&&exChar[i]<='z')||('A'<=exChar[i]&&exChar[i]<='Z')||exChar[i]=='_') {
						sb.append('.');
						while(('a'<=exChar[i]&&exChar[i]<='z')||('A'<=exChar[i]&&exChar[i]<='Z')||exChar[i]=='_') {
							if(i==exChar.length-1) {
								sb.append(exChar[i]);
	...
	public boolean isRightExpression() {
		...
		sym=0;
		index=0;
		//上述三条语句在进行表达式的合法性检测之前还原表达式的初始状态：原因表达式调用一次判断之后不会还原上述三个参数
		next();
		E();
	...
	/**
	 * 只提供get方法，返回Expression表达式对应的词法分析所识别的结果
	 * @return
	 */
	...
	/**
	 * 只提供get方法，用于返回表达式是否有误的编码   0为没有错误 -1为表达式有误
	 */
	...
	public static void main(String[] args) {
		/*
		Experssion e1=new Experssion("a+b+c+f+2+xztdmj");
		Experssion e2=new Experssion("12*(2.5-1.5"+")"+"-10"+")");
